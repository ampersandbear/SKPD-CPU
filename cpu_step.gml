		...
		switch current_step
		{
			case 0: // first, checks
				...
				if (cpu_prevent_backtracking) {
					...
					for (var i = 0; i < _size; i++) {
						var _arr = cpu_prev[| i];
						if (_arr[0] == player.gridx && _arr[1] == player.gridy) _backtracks_amount++;
					}
				...
				pocket_portal			= has_blessing(blessings, blessing.shortcut) && !prefab_is_shop(specific_prefab_loaded);
				...
				with (oUnit) {
					...
					if (_cpu_backtracks_check && _backtracks_amount >= 2 && cpu_added_value > 0) cpu_added_value = 0;
			...
			case 1: // scan all tiles
				if (player.gridy >= 0) {
					...
					if (cpu_mole_b || has_behavior(player, "active_burrow")) {
						for (var i = 0; i < GRID_WIDTH; i++) {
							for (var j = 0; j < GRID_HEIGHT; j++) {
								cpu_scan_tile(i, j, 0, i == player.gridx && j == player.gridy, false, true);
							}
						}
			...
			case 2: // use items / abilities
				...
				if (lvl_advanced_biggies && cpu_attempt_to_avoid_enemy_area_attacks) {
					// dodge griffoth and mole minion attacks:
					with (oWarningFloor) if (unit_instance_is_valid(owner) && owner.grid_master == _gm && (object_index == oUnit and (has_behavior(owner, "slams") || has_behavior(owner, "slams_side")))) {
					...
					with (pUpdateMe) if ((object_index == oLancerJump || object_index == oLancerRaider) && grid_master == _gm) {
						for (var _x = 0; _x < 2 && gridx + _x < GRID_WIDTH; _x++) {
							for (var _y = 0; _y < 2 && gridy + _y < GRID_HEIGHT; _y++) {
			...
			case 3: // choose target
				...
				if (has_modifier(curse.no_loners, player_id) && no_loners_check_softlocked(id, player, -1)) {
				...
				for (var i = ds_list_size(cpu_tiles) - 1; i >= 0; i--) {
					...
					if (_tile[2] > _highest_value) { // found new best
						...
						if (_on_top_of_volleybomb and (_tile[0] < _volleybomb.left || _tile[0] > _volleybomb.right || _tile[1] > _volleybomb.down || _tile[1] < _volleybomb.up)) continue;
						if (!cpu_bomb_grid[# _tile[0], _tile[1]] && !_bomb_resist) continue; // don't pick a target inside the expoding bomb range
					...
					if (cpu_mole_b) {
						...
						for (var i = _volleybomb.gridx - 1; i <= _volleybomb.gridx + 1; i++) {
							for (var e = _volleybomb.gridy - 1; e <= _volleybomb.gridy + 1; e++) {
								...
								var _unit = grid[# i, e];
								if (!unit_instance_is_valid(_unit) || _unit.is_heal) { 
				...
				if (chosen_tile == noone && !_bomb) { 
					...
					if (has_modifier(curse.no_loners, player_id) && !_on_top_of_volleybomb) {
						...
						if (player_gridy < 8 && unit_counts_as_empty(_down)
						&& (player_gridy - 1 < 1 or (unit_instance_is_valid(_up) and (_up.gridy < 1 or chain_size_from_unit(_up,2) <= 1)))
						&& (player_gridx - 1 < 0 or (unit_instance_is_valid(_left) and (_left.gridy < 1 or chain_size_from_unit(_left,2) <= 1)))
						&& (player_gridx + 1 > 7 or (unit_instance_is_valid(_right) and (_right.gridy < 1 or chain_size_from_unit(_right,2) <= 1)))
						) {
